...
min_vals = torch.min(inputs, dim=0).values  # Column-wise minimums
max_vals = torch.max(inputs, dim=0).values  # Column-wise maximums
...
train_loader = DataLoader(train_dataset, batch_size=bsize, shuffle=True,num_workers=31, generator=torch.Generator().manual_seed(seed))
val_loader = DataLoader(val_dataset, batch_size=bsize, shuffle=False,num_workers=31)



class CustomModel_reciprocal_ver1(pl.LightningModule):
...
model_1= CustomModel_reciprocal_ver1(batch_size=bsize,max_lr=max_lr)# learning_rate=lr_myinput)
...
filename_template = f"model_GFX3DNN_catonlytGIF_Mvlognormaldirect_inferGFbGF22_tanh01_{seed}_{job_id}_best{{epoch}}-{{val_loss:.5f}}"
