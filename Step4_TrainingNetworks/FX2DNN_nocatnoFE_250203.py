...
class NN_relu(pl.LightningModule):
    ...
    def forward(self, x):       
        ...
        x = self.pool1(x_conv1) #12-by-13,512 channels
...
if finalactfcn == "relu":
    ...
    filename_template = f"model_FX2DNN_noFE_mvlognmdirect_relu_{seed}_{job_id}_best{{epoch}}-{{val_loss:.5f}}"

elif finalactfcn == "tanh01":
    model_1 = NN_tanh01(batch_size=bsize_myinput, max_lr=max_lr)
    filename_template = f"model_FX2DNN_noFE_mvlognmdirect_tanh01_{seed}_{job_id}_best{{epoch}}-{{val_loss:.5f}}"
