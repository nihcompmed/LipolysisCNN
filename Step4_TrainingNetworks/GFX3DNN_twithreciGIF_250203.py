...
class CustomModel_reciprocal_relu(pl.LightningModule):
    ...
    def forward(self, x):       
        ...
        x = self.pool1(x) #12-by-7,512 channels
...
if finalactfcn == "relu":
    ...
    filename_template = f"model_GFX3DNN_twithreciGIF_mvlognmdirect_npara{npara}_relu_{seed}_{job_id}_best{{epoch}}-{{val_loss:.5f}}"

elif finalactfcn == "tanh01":
    model_1 = CustomModel_reciprocal_tanh01(batch_size=bsize_myinput, max_lr=max_lr)
    filename_template = f"model_GFX3DNN_twithreciGIF_mvlognmdirect_npara{npara}_tanh01_{seed}_{job_id}_best{{epoch}}-{{val_loss:.5f}}"
