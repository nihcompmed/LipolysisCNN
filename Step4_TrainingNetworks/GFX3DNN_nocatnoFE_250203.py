...
npara=int(sys.argv[5])
seed=int(sys.argv[6])
max_epochs=int(sys.argv[7])
ep_maxlr=int(sys.argv[8])
finalactfcn = sys.argv[9]  # Add the new input for the activation function
...
outputs = torch.cat((row_t.unsqueeze(1),row_G.unsqueeze(1),
                    ...
                    # row_1onG.unsqueeze(1),row_1onF.unsqueeze(1)
...
min_per_column = torch.min(Y, dim=0).values
...
train_loader = DataLoader(train_dataset, batch_size=bsize, shuffle=True,num_workers=7, generator=torch.Generator().manual_seed(seed))
val_loader = DataLoader(val_dataset, batch_size=bsize, shuffle=False,num_workers=7)



class CustomModel_reciprocal_relu(pl.LightningModule):
    ...
    def forward(self, x):#500-1-12-16
        #print(x.shape)
        x_conv1 = self.conv1(x) #500-512-6-15
        #print(x_conv1.shape)
        ...
        #x = self.conv2(x_cat) #12-15
        x = self.pool1(x_conv1) #6-by-7,512 channels
        #print(x.shape)
        x = self.conv3(x) #1-by-6,512 channels
        x = self.pool1(x) #1-by-3,512 channels    
        x = self.flatten(x)
        x = self.dense1(x)
        #x = self.relu(x)
        x = torch.tanh(x)
    ...
    def on_train_end(self):
        ...
        #np.savetxt(f'{job_id}_train_losses.csv', np.array(self.train_losses), delimiter=',')
        #np.savetxt(f'{job_id}_val_losses.csv', np.array(self.val_losses), delimiter=',')



class CustomModel_reciprocal_tanh01(pl.LightningModule):
    def __init__(self, batch_size,max_lr):#learning_rate):
        super().__init__()
        #self.learning_rate = learning_rate
        self.batch_size = batch_size
    ...
    def forward(self, x):
        ...
        #       for i in range(x_rep.size(2) + x_pad.size(2))], dim=2) #
        #x = self.conv2(x_cat) #12-15
        x = self.pool1(x_conv1) #12-by-7,512 channels
        #print(x.shape)
        x = self.conv3(x) #1-by-6,512 channels
        #print(x.shape)
        x = self.pool1(x) #1-by-3,512 channels
...
if finalactfcn == "relu":
    model_1 = CustomModel_reciprocal_relu(batch_size=bsize_myinput, max_lr=max_lr)
    filename_template = f"model_GFX3DNN_noFE_mvlognmdirect_npara{npara}_relu_{seed}_{job_id}_best{{epoch}}-{{val_loss:.5f}}"

elif finalactfcn == "tanh01":
    model_1 = CustomModel_reciprocal_tanh01(batch_size=bsize_myinput, max_lr=max_lr)
    filename_template = f"model_GFX3DNN_noFE_mvlognmdirect_npara{npara}_tanh01_{seed}_{job_id}_best{{epoch}}-{{val_loss:.5f}}"
...
#filename_template = "model_GFXNN_twithreciGIF_noinferGIFb_{seed}_{job_id}_best{epoch}-{val_loss:.5f}"
#filename_template = f"model_GFXY4DNN_XminusY_twithreciGIF_AAEgenepara_inferGIFb_tanh01_{seed}_{job_id}_best{{epoch}}-{{val_loss:.5f}}"
